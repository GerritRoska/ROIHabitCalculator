...
const CalculatorPanel: React.FC<CalculatorPanelProps> = ({
  ...
  const [monthlyContribution, setMonthlyContribution] = useState<number>(0); // Not used in calculations
  ...
  const calculateInvestmentGrowth = () => {
    ...
    // Calculate monthly contribution including habit savings
    ...
    for (let year = 0; year <= calculationPeriod; year++) {
      ...
      if (year === 0) {
        ...
        // For subsequent years, calculate using compound interest formula for monthly contributions
        ...
        nominalBalance =
          ...
          effectiveMonthlyContribution *
            ((Math.pow(1 + monthlyNominalRate, months) - 1) /
              monthlyNominalRate);
      ...
      // Calculate base contribution (without habit savings)
      const baseContribution =
        initialInvestment + monthlyContribution * 12 * year;

      // Calculate habit savings contribution
  ...
  const finalAmount = showRealReturns
    ? investmentData[dataIndex].realValue
    : investmentData[dataIndex].value;
  const totalContributions = investmentData[dataIndex].totalContribution;
  const weaknessContribution =
    investmentData[dataIndex].weaknessContribution || 0;
  // Interest earned is the difference between final amount and total contributions
  ...
  const calculateWeaknessMetrics = () => {
    if (showMultipleWeaknesses && weaknessList.length > 0) {
      ...
      // Calculate total items skipped over retirement period
      const getItemCount = (weakness: WeaknessData) => {
        const { timesPerFrequency, frequency } = weakness;
        const frequencyMultiplier =
          frequency === "daily" ? 365 : frequency === "weekly" ? 52 : 12;
        // Use the actual years until retirement for the calculation
      ...
      // Calculate total items over retirement period
      // Use the actual years until retirement for the calculation
      const actualYears = Math.min(yearsUntilRetirement, calculationPeriod);
      const itemCount = Math.floor(
        timesPerFrequency * frequencyMultiplier * actualYears,
      );
...
export default CalculatorPanel;
Code Refactoring: Corrected the for loop, removed unused imports and improved readability.